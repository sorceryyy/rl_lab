...
from model import CnnDQN,Dueling_DQN
from trainer import Trainer
import numpy as np

class CnnDDQNAgent:
    def __init__(self, config: Config, duel_dqn=False):
        self.config = config
        self.is_training = True
        self.buffer = RolloutStorage(config)
        if duel_dqn:
            self.model = Dueling_DQN(self.config.state_shape, self.config.action_dim)
            self.target_model = Dueling_DQN(self.config.state_shape, self.config.action_dim)
        else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dueling', dest='dueling', action='store_true', help='train model')
    ...
    if args.dueling:
        agent = CnnDDQNAgent(config,duel_dqn=True)
    else :
