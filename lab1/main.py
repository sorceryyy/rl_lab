...
class Env(object):
	...
	def step(self, action):
		'''agent act the same action in num_stacks time'''
		reward_sum = 0
...
def main():
	...
	for i in range(num_updates):
		...
		# 	for label_times in range(args.num_steps):
		# 		input_str = "please input label of {}.jpeg:".format(label_times)
		# 		f.writelines(input(input_str)+"\n")
		...
		# 		data_set['label'].append(int(label_tmp.rstrip("\n")))
			

		print("data_len:%d , label_len:%d" %(len(data_set['data']),len(data_set['label'])))
		assert len(data_set['label']) == len(data_set['data'])
		# design how to train your model with labeled data
		agent.update(data_set['data'], data_set['label'])

		if (i + 1) % args.log_interval == 0:
			#time to evaluate your model
			...
			tmp_max=0
			try:
				tmp_mean=np.mean(reward_episode_set)
				tmp_min=np.min(reward_episode_set)
				tmp_max=np.max(reward_episode_set)
			except:
				pass
			else:
				tmp_mean=0
			...
			record['mean'].append(tmp_mean)
			record['max'].append(tmp_max)
			record['min'].append(tmp_mean)
